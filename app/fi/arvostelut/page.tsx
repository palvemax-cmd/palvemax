...
export const metadata: Metadata = {
  ...
  description: "Oikeat arvostelut PalveMax-asiakkailta Riihimäeltä, Hyvinkäältä ja Hämeenlinnasta.",
  alternates: {
              ...
              <div key={t.id} className="card flex flex-col">
                ...
                <div className="pt-3 border-t border-gray-100 flex items-start justify-between gap-2">
                  <div>
                    ...
                    <div className="text-xs text-gray-400 mt-0.5">{t.city} · {t.service}</div>
                  </div>
                  <div className="text-xs text-gray-300 shrink-0">{t.date.split("-").reverse().join(".")}</div>
                </div>
              </div>
            ))}
          </div>

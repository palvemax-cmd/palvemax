...
import { EMAIL, ADDRESS_FULL, ADDRESS_GOOGLE_MAPS, BUSINESS_ID, PHONE_DISPLAY, PHONE_RAW, WHATSAPP_URL } from "@/lib/constants";
...
export default function Footer() {
  ...
  const companyLinks = [
    { href: `/${lang}/${r.about}`, label: t.footer.companyLinks.about },
    { href: `/${lang}/${r.packages}`, label: t.footer.companyLinks.packages },
    { href: `/${lang}/${r.reviews}`, label: t.footer.companyLinks.reviews },
    { href: `/${lang}/${r.cases}`, label: t.footer.companyLinks.cases },
    { href: `/${lang}/${r.blog}`, label: t.footer.companyLinks.blog },
    { href: `/${lang}/${r.gallery}`, label: t.footer.companyLinks.gallery },
  ];
  ...
  return (
    <footer style={{ backgroundColor: "var(--blue-dark)" }} className="text-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              <Image src="/palvemax-logo.png" alt="PalveMax" width={36} height={36}
                className="w-9 h-9 object-contain brightness-0 invert" />
              <div className="font-bold text-lg leading-none" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                PalveMax
              </div>
            </div>
            <p className="text-sm opacity-70 leading-relaxed mb-4">{t.footer.description}</p>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {services.map((s) => (
                <li key={s.slug}>
                  <Link href={`/${lang}/${r.services}/${s.slug}`}
          ...
          <div>
            ...
            <div className="mt-6 pt-6 border-t border-white/10">
              <p className="text-xs opacity-50">{t.footer.workdays}</p>
              <p className="text-xs opacity-50">{t.footer.hours}</p>
              <p className="text-xs opacity-50 mt-2">{t.footer.languages}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        address: {
          "@type": "PostalAddress",
          streetAddress: "Saturnuksenkatu 2 b 18",
          postalCode: "11130",
          addressLocality: "Riihimäki",
          addressRegion: "Kanta-Häme",
          addressCountry: "FI",
        },
        geo: { "@type": "GeoCoordinates", latitude: 60.7397, longitude: 24.7734 },
